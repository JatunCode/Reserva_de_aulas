...
function obtenerDatosSolicitud(button) {
    ...
    fetch('{{ route("admin.reservas.show", ["id" => ":id"]) }}'.replace(':id', id))
        .then(response => response.json())
        .then(data => {
            // console.log("Datos obtenidos:", data); // Imprimir todos los datos obtenidos
...
document.addEventListener("DOMContentLoaded", function() {
    document.getElementById("btnBuscar").addEventListener('click', function() {
        ...
        fetch('{{ route("admin.solicitud.filtrar.datos_filtro") }}?modo=' + modo + '&estado=' + estado + '&materia=' + materia)
            .then(response => response.json())
            .then(data => {
